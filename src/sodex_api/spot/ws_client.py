...
from .models import (
    Orderbook, OBItem, TradeData, TickerData, UserBalanceData, 
    UserOrderData, UserTradeData, SystemMessage, KlineStreamData, DepthData
)
...
class SpotWebSocketClient:
    """WebSocket client for Sodex Exchange spot trading real-time data."""
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            self.auth_token = self.spot_client.get_websocket_token()
    ...
    async def subscribe_symbol(self, symbol: str, callback: Callable[[Union[DepthData, Orderbook, TradeData]], None]):
        ...
        sodex_symbol = self.spot_client._convert_symbol(symbol)
    ...
    async def _handle_depth_data(self, data: Dict[str, Any]):
        ...
        try:
            depth = DepthData(
                id=data.get("id", ""),
                symbol=self.spot_client._convert_symbol(data.get("s", ""), reverse=True),
    ...
    async def _handle_user_order_data(self, data: Dict[str, Any]):
        ...
        try:
            order = UserOrderData(
                ...
                symbol=self.spot_client._convert_symbol(data.get("symbol", ""), reverse=True),
